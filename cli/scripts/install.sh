#!/bin/bash
#
# Reward center CLI binary installation script
# ------------------------------------
#
...
CYN  "Reward center CLI binary installation script"
echo "---------------------------------------"
echo ""
...
case "$PROCESSOR" in
    arm* | aarch* | ppc* )
        if [ "$OS_FLAVOUR" != Darwin ]; then
            ...
            echo "  => $(CYN https://github.com/holaplex/reward-center-program/tree/main/cli)"
            echo ""
            echo "to build Reward center from the source code."
            exit 1
        fi
        ;;
...
RELEASE_URL="https://github.com/holaplex/reward-center-program/releases"
RELEASE="latest"
BIN="Reward center"
...
if [ $SIZE -eq 0 ]; then
    RED "Aborting: could not download Reward center distribution"
    exit 1
fi
...
if [ ! "$(command -v $BIN)" = "" ]; then
    ...
    echo "Reward center binary was found at:"
    echo "  => $(CYN $EXISTING)"
    echo ""
    ...
    if [ "$(command -v $BIN)" = "" ]; then
        ...
        if [ -f "$ENV_FILE" ]; then
            ...
            echo "  => adding '$TARGET' to 'PATH' variable to execute 'Reward center' from any directory."
...
# sanity check
if [ "$(command -v $BIN)" = "" ]; then
    # installation was completed, but Reward center is not in the PATH
